...
export const signup = async (req, res, next) => {
    try {
        ...
        const token = jwttoken.sign({ id: user.newUser.id, email: user.newUser.email, role: user.newUser.role });
        ...
        res.status(201).json({
            ...
            user: { id: user.newUser.id, name: user.newUser.name, email: user.newUser.email, role: user.newUser.role }
        });
